...
// than Shiprocket's full raw payload. Returns null if Shiprocket has no shipment
// matching this AWB under the connected account.
...
function parseTrackingResponse(data) {
  ...
  // Shiprocket returns HTTP 200 even when the AWB doesn't exist under this account —
  // the actual signal is an `error` string and/or an empty current_status.
  if (!shipment || track?.error || !shipment.current_status) return null;
