...
export const useCart = create(
  persist(
    (set, get) => ({
      items: [], // { id, name, price, image, size, qty, sizes }
      ...
      // Change the size of a cart line. If another line already has that size, merge quantities.
      changeSize: (id, oldSize, newSize) => {
        if (oldSize === newSize) return;
        const items = get().items;
        const line = items.find((i) => i.id === id && i.size === oldSize);
        if (!line) return;
        const clash = items.find((i) => i.id === id && i.size === newSize);
        if (clash) {
          set({
            items: items
              .map((i) => (i.id === id && i.size === newSize ? { ...i, qty: i.qty + line.qty } : i))
              .filter((i) => !(i.id === id && i.size === oldSize)),
          });
        } else {
          set({
            items: items.map((i) => (i.id === id && i.size === oldSize ? { ...i, size: newSize } : i)),
          });
        }
      },
      remove: (id, size) =>
