...
//
// Prefers a dedicated ADMIN_SESSION_SECRET, falling back to the service-role
// key so existing sessions keep working where that variable isn't set yet.
// Signing cookies with the database key works, but it couples two unrelated
// secrets — rotating the Supabase key would silently log out every admin.
//
// There is deliberately no hardcoded production fallback. This previously
// defaulted to the literal string 'dev-secret', which meant a missing env var
// in production would sign admin sessions with a value published in this
// repository — anyone reading it could forge an admin cookie. Refusing to
// boot is strictly better than serving a forgeable admin session.
...
function verifySession(token) {
  ...
  // Constant-time comparison, matching how verifyAdminPassword compares
  // hashes. Lengths must match before timingSafeEqual will accept the inputs.
